...
class StatusRepositoryTest {
    ...
    void update() throws Exception {
        ControllerStatusReceived first =
                ControllerStatusReceived.with(ControllerStatus.RUN, VesselId.of(1), UUID.randomUUID());

        repository.save(first);

        ControllerStatusReceived second =
                ControllerStatusReceived.with(ControllerStatus.STOP, VesselId.of(1), UUID.randomUUID());
